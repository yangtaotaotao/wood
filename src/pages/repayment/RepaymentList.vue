<template>
  <div class="customer-wrapper car-column car-bg-white">
    ...
    <div class="scroll-content car-column" ref="scroller">
      <repayment-list-item v-for="(item, index) in repaymentArr" :item="item" :key="index" @toDetail="toDetail(item)"></repayment-list-item>
...
import { queryListRepaymentPlan, isLogined } from '@/api/getData.js'
const hasNextPageCount = 10
export default {
  name: 'repayment-list',

  components: {
    SearchInput, RepaymentListItem, InfiniteScroll
  },
  ...
  methods: {
    ...
    toDetail (item) {
      this.$router.push({name: 'repaymentInfo', params: {repaymentId: item.id}})
    },
    _mock () {
      this.loading = true
      isLogined().then(res => {
          ...
          queryListRepaymentPlan({
            ...
            if (success) {
              if (data.records.length > 0) {
                ...
                this.$store.dispatch('setToast', {text: '暂无还款信息'})
              }
            }
          }).catch(err => {
            this.loading = false
