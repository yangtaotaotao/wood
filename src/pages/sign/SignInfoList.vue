...
import LoanInfoListItem from '../loanInfo/LoanInfoListItem'
...
export default {
  ...
  methods: {
    ...
    // 详情
    toDetail (item) {
      if (this.userAppCode === 'WOOD_APP') {
        this.$router.push({name: 'signInfo', params: {signId: item.id}})
      } else if (this.userAppCode === 'AUTO_APP') {
        this.$router.push({name: 'signInfoCar', params: {signId: item.id}})
      }
    },
    _mock () {
      this.loading = true
      isLogined().then(res => {
        if (!res.data.success) {
          ...
          this.userAppCode = res.data.data.userAppCode
          loanQueryList({
            pageSize: 10,
